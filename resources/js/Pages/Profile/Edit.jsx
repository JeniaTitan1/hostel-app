...
export default function Edit({ auth, mustVerifyEmail, status }) {
    return (
        ...
        >
            ...
            <div className="py-8 bg-gray-50 min-h-[calc(100vh-73px)]">
                <div className="mx-auto max-w-7xl space-y-6 px-4 sm:px-6 lg:px-8">
                    {auth?.user?.must_change_password && (
                        <div className="bg-amber-50 border-l-4 border-amber-500 p-4 rounded-xl shadow-xs space-y-2">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="font-bold text-amber-800 text-sm">Обов'язкова первинна реєстрація</span>
                            </div>
                            ...
                            <div className="flex flex-wrap gap-x-4 gap-y-1 pt-1 text-[11px] font-bold">
                                <span className={auth?.user?.name && !auth?.user?.name.startsWith('Тимчасовий') && !auth?.user?.name.startsWith('Temporary') ? 'text-emerald-700 flex items-center gap-1' : 'text-red-600 flex items-center gap-1'}>
                                    {auth?.user?.name && !auth?.user?.name.startsWith('Тимчасовий') && !auth?.user?.name.startsWith('Temporary') ? '✓' : '✗'} Тимчасове Ім'я (потрібно змінити)
                                </span>
                                <span className={auth?.user?.email && !(auth?.user?.email.startsWith('student') && auth?.user?.email.endsWith('@mnau.edu.ua')) ? 'text-emerald-700 flex items-center gap-1' : 'text-red-600 flex items-center gap-1'}>
                                ...
                                <span className={auth?.user?.telegram ? 'text-emerald-700 flex items-center gap-1' : 'text-gray-500 flex items-center gap-1 font-normal'}>
                                    {auth?.user?.telegram ? '✓' : '○'} Telegram (необов'язково)
                                </span>
                                <span className={auth?.user?.phone ? 'text-emerald-700 flex items-center gap-1' : 'text-red-600 flex items-center gap-1'}>
                                    {auth?.user?.phone ? '✓' : '✗'} Телефон ({auth?.user?.phone || 'немає'})
                                </span>
                                <span className={auth?.user?.password_changed ? 'text-emerald-700 flex items-center gap-1' : 'text-red-600 flex items-center gap-1'}>
                    ...
                    {!auth?.user?.must_change_password && auth?.user?.password_changed && (
                        <div className="bg-emerald-50 border-l-4 border-emerald-500 p-4 rounded-xl shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4 animate-fade-in">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h4 className="font-bold text-emerald-800 text-sm">Реєстрація успішно завершена!</h4>
                                    <p className="text-xs text-emerald-700">Всі обов'язкові поля заповнено та пароль оновлено. Доступ до сайту відкрито.</p>
                                </div>
                            </div>
                            <Link
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        {/* Редагування профілю */}
                        <div className="lg:col-span-2 bg-white border border-gray-200 p-6 sm:p-8 rounded-xl shadow-xs">
                        ...
                        <div className="space-y-6">
                            <div className="bg-white border border-gray-200 p-6 sm:p-8 rounded-xl shadow-xs">
                                <UpdatePasswordForm />
                            </div>
